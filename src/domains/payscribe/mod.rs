...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/payscribe/electricity/validate",
            post(validate_electricity_customer),
        )
...
async fn payscribe_status() -> Json<PayscribeStatusResponse> {
    let configured = !std::env::var("PAYSCRIBE_API_KEY")
        .unwrap_or_default()
        .trim()
        .is_empty();
    Json(PayscribeStatusResponse {
        ...
        message: if configured {
            "Payscribe backend is configured.".to_string()
        } else {
...
async fn data_lookup(
    ...
    if !network.is_empty() && !is_supported_data_network(&network) {
        return Err(GatewayError::Upstream(
            "unsupported data network".to_string(),
        ));
    }
    ...
    Ok(Json(
        json!({ "accepted": status.is_success(), "network": if network.is_empty() { Value::Null } else { Value::String(network) }, "providerStatus": status.as_u16(), "providerResponse": body }),
    ))
}
...
async fn data_quote(
    ...
    let plan_amount = find_plan_amount(&lookup, &plan).ok_or_else(|| {
        GatewayError::Upstream(
            "selected data plan was not found in Payscribe lookup response".to_string(),
        )
    })?;
    ...
    Ok(Json(DataQuoteResponse {
        ...
        plan_amount,
        charge_credits,
        pricing_policy: json!({ "creditsPerNaira": payscribe_data_credits_per_naira(), "serviceFeeCredits": payscribe_data_service_fee_credits() }),
    }))
}
...
async fn validate_electricity_customer(
    ...
    let service = request
        ...
        .unwrap_or_default();
    let meter_number = request
        .meter_number
        .clone()
        .or(request.meter_no.clone())
        .unwrap_or_default();
    if service.trim().is_empty() {
        return Err(GatewayError::Upstream(
            "service/disco/provider is required for electricity validation".to_string(),
        ));
    }
    if meter_number.trim().is_empty() {
        return Err(GatewayError::Upstream(
            "meterNumber is required for electricity validation".to_string(),
        ));
    }
    let payload = json!({
        "meter_number": meter_number,
        "meter_type": request.meter_type.unwrap_or_else(|| "prepaid".to_string()),
        "amount": request.amount.unwrap_or(1000.0).to_string(),
        "service": service,
        "customer_phone": request.customer_phone
    });
    let response = payscribe_post(
        &state,
        &payscribe_electricity_validate_path(),
        payload.clone(),
    )
    .await?;
    let status = response.status();
    let body: Value = response.json().await?;
    Ok(Json(
        json!({ "accepted": status.is_success(), "providerStatus": status.as_u16(), "requestPayload": payload, "providerResponse": body }),
    ))
}

async fn electricity_quote(
    State(state): State<AppState>,
    Json(request): Json<ElectricityQuoteRequest>,
) -> GatewayResult<Json<ElectricityQuoteResponse>> {
    validate_electricity_fields(
        &request.service,
            ...
            .unwrap_or(false)
    {
        return Err(GatewayError::Upstream(format!(
            "Payscribe electricity validation failed: {validation}"
        )));
    }
    ...
    Ok(Json(ElectricityQuoteResponse {
        ...
        charge_credits,
        validation,
        pricing_policy: json!({ "creditsPerNaira": payscribe_electricity_credits_per_naira(), "serviceFeeCredits": payscribe_electricity_service_fee_credits() }),
    }))
}
...
async fn vend_data(
    ...
    let provider_reference = request
        .ref_id
        .clone()
        .unwrap_or_else(|| format!("ps_data_{}", Uuid::new_v4().simple()));
    let lookup = fetch_data_lookup(&state, &network).await?;
    let plan_amount = find_plan_amount(&lookup, &plan).ok_or_else(|| {
        GatewayError::Upstream(
            "selected data plan was not found in Payscribe lookup response".to_string(),
    ...
    let provider_trans_id = body["message"]["details"]["trans_id"]
        .as_str()
        .map(str::to_string);
    ...
    Ok(Json(
        insert_payscribe_transaction(
            ...
            Some(&network),
            Some(&plan),
            Some(payload.get("recipient").cloned().unwrap_or(Value::Null)),
            charge_credits,
            &provider_status,
            provider_trans_id,
            body,
...
async fn vend_electricity(
    State(state): State<AppState>,
    Json(request): Json<ElectricityVendRequest>,
) -> GatewayResult<Json<PayscribeTransactionRecord>> {
    validate_electricity_fields(
        &request.service,
        &request.meter_number,
    ...
    if request.customer_name.trim().is_empty() {
        return Err(GatewayError::Upstream(
            "customerName is required from electricity validation response".to_string(),
        ));
    }
    let provider_reference = request
        .ref_id
        .clone()
        .unwrap_or_else(|| format!("ps_elec_{}", Uuid::new_v4().simple()));
    let charge_credits = calculate_electricity_charge_credits(request.amount);
    ...
    let response =
        payscribe_post(&state, &payscribe_electricity_vend_path(), payload.clone()).await?;
    let status = response.status();
    let body: Value = response.json().await?;
    let provider_status = extract_transaction_status(&body, status.as_u16());
    let provider_trans_id = body["message"]["details"]["trans_id"]
    ...
    if !(status.is_success() && body.get("status").and_then(Value::as_bool).unwrap_or(false)) {
        let _ = credit_credits(
            ...
            request.account_id,
            charge_credits,
            "payscribe_electricity_reversal",
            &provider_reference,
            "Reversal for rejected Payscribe electricity vend",
            json!({ "providerResponse": body }),
        )
        .await;
    }
...
async fn insert_payscribe_transaction(
    state: &AppState,
    account_id: Option<Uuid>,
    service_type: &str,
    ...
    provider_trans_id: Option<String>,
    provider_payload: Value,
) -> GatewayResult<PayscribeTransactionRecord> {
...
fn validate_electricity_fields(
    service: &str,
    meter_number: &str,
    meter_type: &str,
    amount: f64,
) -> GatewayResult<()> {
    if !is_supported_disco(service.trim().to_lowercase().as_str()) {
        return Err(GatewayError::Upstream(
            "unsupported electricity service/disco".to_string(),
        ));
    }
    if meter_number.trim().is_empty() {
        return Err(GatewayError::Upstream(
            "meterNumber is required".to_string(),
        ));
    }
    if !matches!(
        meter_type.trim().to_lowercase().as_str(),
        "prepaid" | "postpaid"
    ) {
        return Err(GatewayError::Upstream(
            "meterType must be prepaid or postpaid".to_string(),
        ));
    }
    if !amount.is_finite() || amount < 1000.0 {
        return Err(GatewayError::Upstream(
            "electricity amount must be at least NGN 1,000".to_string(),
        ));
    }
    Ok(())
}
...
fn is_supported_data_network(network: &str) -> bool {
    matches!(
        network,
        "mtn" | "glo" | "airtel" | "9mobile" | "smile" | "dstvshowmax"
    )
}
fn is_supported_disco(service: &str) -> bool {
    matches!(
        service,
        "ikedc" | "ekedc" | "eedc" | "phedc" | "aedc" | "ibedc" | "kedco" | "jed"
    )
}
...
async fn fetch_data_lookup(state: &AppState, network: &str) -> GatewayResult<Value> {
    ...
    if !status.is_success() {
        return Err(GatewayError::Upstream(format!(
            "Payscribe data lookup failed: {body}"
        )));
    }
    Ok(body)
}
...
fn payscribe_data_credits_per_naira() -> f64 {
    env_f64("PAYSCRIBE_DATA_CREDITS_PER_NAIRA", 1.0, true)
}
fn payscribe_data_service_fee_credits() -> f64 {
    env_f64("PAYSCRIBE_DATA_SERVICE_FEE_CREDITS", 0.0, false)
}
fn payscribe_electricity_credits_per_naira() -> f64 {
    env_f64("PAYSCRIBE_ELECTRICITY_CREDITS_PER_NAIRA", 1.0, true)
}
fn payscribe_electricity_service_fee_credits() -> f64 {
    env_f64("PAYSCRIBE_ELECTRICITY_SERVICE_FEE_CREDITS", 0.0, false)
}
fn calculate_data_charge_credits(plan_amount: f64) -> f64 {
    round_2(plan_amount * payscribe_data_credits_per_naira() + payscribe_data_service_fee_credits())
}
fn calculate_electricity_charge_credits(amount: f64) -> f64 {
    round_2(
        amount * payscribe_electricity_credits_per_naira()
            + payscribe_electricity_service_fee_credits(),
    )
}
fn env_f64(key: &str, fallback: f64, must_be_positive: bool) -> f64 {
    std::env::var(key)
        .ok()
...
fn find_plan_amount(value: &Value, plan_code: &str) -> Option<f64> {
    match value {
        Value::Object(map) => {
            ...
            map.values()
                .find_map(|child| find_plan_amount(child, plan_code))
        }
...
fn payscribe_base_url() -> String {
    std::env::var("PAYSCRIBE_BASE_URL")
        .unwrap_or_else(|_| "https://sandbox.payscribe.ng/api/v1".to_string())
}
fn payscribe_electricity_validate_path() -> String {
    std::env::var("PAYSCRIBE_ELECTRICITY_VALIDATE_PATH")
        .unwrap_or_else(|_| "/electricity/validate".to_string())
}
fn payscribe_electricity_vend_path() -> String {
    std::env::var("PAYSCRIBE_ELECTRICITY_VEND_PATH")
        .unwrap_or_else(|_| "/electricity/vend".to_string())
}
fn payscribe_token() -> GatewayResult<String> {
    ...
    if token.trim().is_empty() {
        return Err(GatewayError::Config(
            "PAYSCRIBE_API_KEY is required".to_string(),
        ));
    }
    Ok(token)
}
fn extract_transaction_status(body: &Value, http_status: u16) -> String {
    ...
    if let Some(status) = body["message"]["details"]["status"].as_str() {
        return status.to_string();
    }

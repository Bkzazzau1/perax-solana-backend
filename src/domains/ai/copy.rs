...
async fn copy_generate(
    ...
    let reference = payload.ref_id.clone().unwrap_or_else(|| format!("copy_ai_{}", Uuid::new_v4().simple()));
    ...
    let outputs = (1..=variants).map(|i| render_copy(&payload, i)).collect::<Vec<_>>();

    Ok(Json(CopyGenerateResponse {
        accepted: true,
        reference,
        copy_kind: payload.copy_kind,
        title: format!("{} for {}", payload.copy_kind.label(), payload.product_or_service),
        credit_cost,
        outputs,
    }))
}
...
async fn unit_cost(state: &AppState) -> GatewayResult<f64> {
    Ok(pricing::get_utility_price(state, "copy_ai_generate").await?.credit_cost)
}

fn normalize_variants(value: Option<i32>) -> GatewayResult<i32> {
    let variants = value.unwrap_or(1);
    if !(1..=5).contains(&variants) {
        return Err(GatewayError::Upstream("variants must be between 1 and 5".to_string()));
    }
    Ok(variants)
}
...
fn validate_request(payload: &CopyGenerateRequest) -> GatewayResult<()> {
    if payload.product_or_service.trim().is_empty() {
        return Err(GatewayError::Upstream("productOrService is required".to_string()));
    }
    if payload.product_or_service.chars().count() > 200 {
        return Err(GatewayError::Upstream("productOrService is too long".to_string()));
    }
    if payload.key_points.as_ref().is_some_and(|items| items.len() > 8) {
        return Err(GatewayError::Upstream("keyPoints cannot exceed 8 items".to_string()));
    }
    Ok(())
}
...
fn render_copy(payload: &CopyGenerateRequest, index: i32) -> String {
    ...
    let cta = payload.call_to_action.as_deref().unwrap_or("Get started today");
    let points = payload.key_points.as_ref()
        .map(|items| items.iter().filter(|item| !item.trim().is_empty()).cloned().collect::<Vec<_>>().join(", "))
        .filter(|text| !text.trim().is_empty())
        .unwrap_or_else(|| "simple value, reliable service, and a smooth customer experience".to_string());

    match payload.copy_kind {
        CopyKind::AdCopy => format!("Variant {index}: {business} helps {audience} enjoy {} with {points}. Built for {platform} in a {tone} tone. {cta}.", payload.product_or_service),
        CopyKind::SocialCaption => format!("Variant {index}: Need {} that feels simple and reliable? {business} brings you {points}. {cta}.", payload.product_or_service),
        CopyKind::ProductDescription => format!("Variant {index}: {} by {business} is designed for {audience}. It focuses on {points} with a {tone} experience.", payload.product_or_service),
        CopyKind::EmailCopy => format!("Subject: A better way to use {}\n\nHello,\n\n{business} helps {audience} get more value from {}. Key benefits include {points}.\n\n{cta}.", payload.product_or_service, payload.product_or_service),
        CopyKind::SmsCopy => format!("{business}: Get {} with {points}. {cta}.", payload.product_or_service),
        CopyKind::LandingHero => format!("{} that helps {audience} move faster\n\n{business} gives you {points} with a {tone} experience.\n\n{cta}.", payload.product_or_service),
        CopyKind::BusinessBio => format!("{business} provides {} for {audience}. The brand focuses on {points} and delivers a {tone} customer experience.", payload.product_or_service),
    }
}

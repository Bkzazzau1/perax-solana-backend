...
async fn copyleaks_status() -> Json<CopyleaksStatusResponse> {
    ...
    Json(CopyleaksStatusResponse {
        ...
        mode: if configured { "ready" } else { "configuration_required" }.to_string(),
        message: if configured {
            "Copyleaks premium scan foundation is configured.".to_string()
        } else {
            "COPYLEAKS_EMAIL and COPYLEAKS_API_KEY are required before live Copyleaks scans.".to_string()
        },
    })
}
...
async fn copyleaks_quote(
    ...
    let credit_cost = pricing::get_utility_price(&state, "copyleaks_premium_scan").await?.credit_cost;
...
async fn copyleaks_submit(
    ...
    if payload.text.trim().is_empty() {
        return Err(GatewayError::Upstream("text is required for Copyleaks scan".to_string()));
    }
    if payload.text.chars().count() < 50 {
        return Err(GatewayError::Upstream("text must be at least 50 characters for a premium scan".to_string()));
    }

    let scan_type = normalize_scan_type(payload.scan_type.as_deref());
    let credit_cost = pricing::get_utility_price(&state, "copyleaks_premium_scan").await?.credit_cost;
    let reference = payload.ref_id.clone().unwrap_or_else(|| format!("copyleaks_{}", Uuid::new_v4().simple()));
    ...
    let submit_response = submit_to_copyleaks(&state, &scan_id, submit_payload.clone()).await.unwrap_or_else(|err| json!({
        "submitted": false,
        "deferred": true,
        "error": err.to_string(),
        "note": "Scan was recorded and Credits were debited, but live Copyleaks submission needs valid credentials and payload confirmation."
    }));

    let status = if submit_response.get("submitted").and_then(Value::as_bool).unwrap_or(false) { "submitted" } else { "created" };
...
async fn copyleaks_webhook(
    ...
    let payload: Value = serde_json::from_slice(&body).map_err(|_| GatewayError::Upstream("invalid Copyleaks webhook JSON".to_string()))?;
    let scan_id = payload.get("scannedDocument").and_then(|v| v.get("scanId")).and_then(Value::as_str)
    ...
    Ok(Json(json!({ "accepted": true, "matchedRows": updated.rows_affected(), "scanId": scan_id })))
}
...
async fn submit_to_copyleaks(state: &AppState, scan_id: &str, payload: Value) -> GatewayResult<Value> {
    let token = copyleaks_token(state).await?;
    let url = format!("{}/v3/scans/submit/file/{}", copyleaks_base_url(), scan_id);
    let response = state.http.put(url).bearer_auth(token).json(&payload).send().await?;
...
async fn copyleaks_token(state: &AppState) -> GatewayResult<String> {
    ...
    if email.trim().is_empty() || key.trim().is_empty() {
        return Err(GatewayError::Config("COPYLEAKS_EMAIL and COPYLEAKS_API_KEY are required".to_string()));
    }
    let response = state.http.post(format!("{}/v3/account/login/api", copyleaks_base_url()))
        .json(&json!({ "email": email, "key": key }))
        .send()
        .await?;
    let body: Value = response.json().await?;
    body.get("access_token").and_then(Value::as_str).map(str::to_string)
        .or_else(|| body.get("accessToken").and_then(Value::as_str).map(str::to_string))
        .ok_or_else(|| GatewayError::Upstream(format!("Copyleaks token missing from response: {body}")))
}
...
fn verify_webhook(headers: &HeaderMap) -> GatewayResult<()> {
    ...
    let provided = headers.get("x-perax-webhook-secret").and_then(|value| value.to_str().ok()).unwrap_or_default();
    if provided == configured { Ok(()) } else { Err(GatewayError::Unauthorized) }
}

...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/payments/paystack/virtual-account/assign",
            post(assign_virtual_account),
        )
        .route(
            "/payments/paystack/virtual-account/mine",
...
async fn initialize_transaction(
    ...
    let channels = request.channels.unwrap_or_else(|| {
        if intent.funding_method == "virtual_account" {
            vec!["bank_transfer".to_string()]
        } else {
            vec!["card".to_string()]
        }
    });
    ...
    if !status.is_success() || !body.get("status").and_then(Value::as_bool).unwrap_or(false) {
        return Err(GatewayError::Upstream(format!(
            "Paystack initialize failed: {body}"
        )));
    }
...
async fn verify_transaction(
    ...
    verify_paystack_reference(
        &state,
        request.intent_reference.as_deref(),
        &request.reference,
    )
    .await
}
...
async fn receive_webhook(
    ...
    let payload: Value = serde_json::from_slice(&body)
        .map_err(|_| GatewayError::Upstream("Paystack webhook JSON is invalid".to_string()))?;
    let event = payload
        .get("event")
    ...
    if event != "charge.success" || reference.trim().is_empty() {
        return Ok(Json(
            json!({ "accepted": true, "processed": false, "event": event }),
        ));
    }

    let verified = verify_paystack_reference(&state, None, reference).await?;
    Ok(Json(
        json!({ "accepted": true, "processed": true, "verification": verified.0 }),
    ))
}
...
async fn assign_virtual_account(
    ...
    if !status.is_success() || !body.get("status").and_then(Value::as_bool).unwrap_or(false) {
        return Err(GatewayError::Upstream(format!(
            "Paystack virtual account assign failed: {body}"
        )));
    }
    let data = body.get("data").unwrap_or(&Value::Null);
    Ok(Json(
        upsert_virtual_account(
            &state,
            request.account_id,
            &request.email,
            data,
...
async fn verify_paystack_reference(
    state: &AppState,
    intent_reference: Option<&str>,
    reference: &str,
) -> GatewayResult<Json<Value>> {
    let response =
        paystack_get(state, &format!("/transaction/verify/{}", reference.trim())).await?;
    let status = response.status();
    let body: Value = response.json().await?;
    if !status.is_success() || !body.get("status").and_then(Value::as_bool).unwrap_or(false) {
        return Err(GatewayError::Upstream(format!(
            "Paystack transaction verify failed: {body}"
        )));
    }
    let data = body.get("data").unwrap_or(&Value::Null);
    if data
        ...
        != "success"
    {
        return Err(GatewayError::Upstream(
            "Paystack transaction is not successful".to_string(),
        ));
    }
    ...
    let api_base = std::env::var("PERAX_INTERNAL_API_BASE_URL")
    ...
    let response = state
        .http
        .post(format!("{api_base}/payments/verify/provider"))
        .json(&verify_payload)
        .send()
        .await?;
    let provider_body: Value = response.json().await?;
    Ok(Json(
        json!({ "paystackVerified": true, "providerVerification": provider_body }),
    ))
}

async fn find_intent_by_reference(
    state: &AppState,
    intent_reference: &str,
) -> GatewayResult<PaymentIntentForPaystack> {
    find_intent(state, "intent_reference = $1", intent_reference.trim()).await
}

async fn find_intent_by_provider_reference(
    state: &AppState,
    reference: &str,
) -> GatewayResult<PaymentIntentForPaystack> {
...
async fn find_intent(
    ...
    let sql = format!(
        "select id, intent_reference, quote_reference, funding_method, provider_reference, expected_asset_code, expected_amount::float8 as expected_amount, expected_usd_value::float8 as expected_usd_value, expected_credits::float8 as expected_credits, status from payment_intents where {where_clause} limit 1"
    );
    sqlx::query_as::<_, PaymentIntentForPaystack>(&sql)
        .bind(value)
        .fetch_optional(&state.db)
        .await?
        .ok_or_else(|| {
            GatewayError::Upstream("payment intent not found for Paystack flow".to_string())
        })
}
...
fn ensure_paystack_supported_intent(intent: &PaymentIntentForPaystack) -> GatewayResult<()> {
    ...
    if !matches!(intent.funding_method.as_str(), "card" | "virtual_account") {
        return Err(GatewayError::Upstream(
            "Paystack only supports card and Nigerian virtual-account deposits".to_string(),
        ));
    }
    Ok(())
}
...
fn usd_to_paystack_kobo(_state: &AppState, amount_usd: f64) -> GatewayResult<i64> {
    let rate = std::env::var("PAYSTACK_NGN_PER_USD")
        .unwrap_or_else(|_| "1500".to_string())
        .parse::<f64>()
        .map_err(|_| GatewayError::Config("PAYSTACK_NGN_PER_USD must be numeric".to_string()))?;
    if amount_usd <= 0.0 || rate <= 0.0 {
        return Err(GatewayError::Config(
            "PAYSTACK_NGN_PER_USD must be positive".to_string(),
        ));
    }
    Ok((amount_usd * rate * 100.0).round() as i64)
}

fn validate_paystack_amount(
    state: &AppState,
    intent: &PaymentIntentForPaystack,
    data: &Value,
) -> GatewayResult<()> {
    ...
    if currency != "NGN" {
        return Err(GatewayError::Upstream(
            "Paystack transaction currency must be NGN".to_string(),
        ));
    }
    if paid_kobo < usd_to_paystack_kobo(state, intent.expected_amount)? {
        return Err(GatewayError::Upstream(
            "Paystack amount is less than expected quote amount".to_string(),
        ));
    }
    Ok(())
}
...
fn verify_paystack_webhook(
    ...
    if secret.trim().is_empty() {
        return Err(GatewayError::Config(
            "PAYSTACK_SECRET_KEY is required for Paystack webhook verification".to_string(),
        ));
    }
    ...
    let mut mac = HmacSha512::new_from_slice(secret.as_bytes())
        .map_err(|_| GatewayError::Config("invalid PAYSTACK_SECRET_KEY".to_string()))?;
    mac.update(body);
...
async fn paystack_post(
    ...
    if secret.trim().is_empty() {
        return Err(GatewayError::Config(
            "PAYSTACK_SECRET_KEY is required".to_string(),
        ));
    }
    let base = std::env::var("PAYSTACK_BASE_URL")
...
async fn upsert_virtual_account(
    state: &AppState,
    account_id: Uuid,
    email: &str,
    data: &Value,
    provider_payload: Value,
) -> GatewayResult<PaystackVirtualAccountRecord> {
    let customer_code = data["customer"]["customer_code"]
        .as_str()
        .or_else(|| data["customer_code"].as_str())
        .map(str::to_string);
    ...
    let bank_name = data["bank"]["name"]
        .as_str()
        .or_else(|| data["bank_name"].as_str())
        .map(str::to_string);
    let bank_slug = data["bank"]["slug"]
        .as_str()
        .or_else(|| data["bank_slug"].as_str())
        .map(str::to_string);
    let currency = data["currency"].as_str().unwrap_or("NGN").to_string();
    let provider_status = if data["active"].as_bool().unwrap_or(true) {
        "active"
    } else {
        "inactive"
    };
...
async fn log_paystack_transaction(
    state: &AppState,
    action: &str,

...
worker = read("src/domains/checkout/worker.rs")
main = read("src/main.rs")
migration = read("migrations/20260718190000_create_checkout_settlement_orders.sql")
worker_migration = read("migrations/20260718193000_add_checkout_settlement_worker.sql")
...
require(worker, "for update skip locked", "settlement worker")
require(worker, "settlement_claimed_at", "settlement worker")
require(worker, "PERAX_SETTLEMENT_EXECUTOR_URL", "settlement worker")
require(worker, "verify_finalized_on_chain", "settlement worker")
require(worker, '"getSignatureStatuses"', "settlement worker")
require(worker, '"getAccountInfo"', "settlement worker")
require(worker, "owner != state.config.perax_program_id", "settlement worker")
require(worker, "checkout_refund", "settlement worker")
require(worker, "terminal_failure", "settlement worker")
require(worker, "refund_ledger_id.is_some()", "settlement worker")
require(worker, "pg_advisory_xact_lock", "settlement worker")
require(main, "spawn_checkout_settlement_worker", "main startup")
...
require(worker_migration, "settlement_attempt_count", "worker migration")
require(worker_migration, "refund_ledger_id", "worker migration")
require(worker_migration, "idx_checkout_settlement_orders_worker_claim", "worker migration")
